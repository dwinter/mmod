...
\texttt{Gst} is the standard (Nei) G$_{ST}$, \verb+Gprime_st+ is Hedrick's G''$_{ST}$ and \texttt{D}
 is Jost's d$_{EST}$.
 
d$_{EST}$ is the easiest statistic to interpret, as you expect to find $D=0$ for
...
divergence but they range from ~0.1--0.4. I'd say that's quite 
substantial. 
\newpage
...
average of \texttt{H$_S$} and \texttt{H$_T$} across loci. For d$_{EST}$ you get two, the harmonic mean of the
d$_{EST}$ for each locus and, because that method won't work if you end up with
...
\texttt{str(bs.D))} or the classic bs.D\$ [tab tab] to check it out. I don't think there
is much point trying to interpret confidence intervals estimated from 10 
samples, so let's move on.
\newpage
...
use the "linearized" version of each D, which is just $x/(1-x)$.
