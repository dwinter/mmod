...
D is the easiest statistic to interpret, as you expect to find $D=0$ for
...
\newpage
You probably want to have some idea of how robust this result is. Their are a 
...
sample of the \texttt{nancycats} data.
<<bs>>=
...
overview of that data, but there is quite a lot more there use 
\texttt{str(bs.D))} to check it out. I don't think there
