...
``F statistics'' (measures based on Wrigtht's $F_\text{ST}$) to compare diversity
within and between predefined sub-populations. Until recently, the most widely 
used of these statistics has been Nei's $G_\text{ST}$. Unfortunately, it has 
become increasingly clear that the value of $G_\text{ST}$ is a at least 
partially dependent on the number of alleles at each locus and the number of 
populations sampled - making simple interpretation of this statistics 
difficult. 

A number of ``$F_\text{ST}$ analogues'' have been developed that compensate for 
...
Meirmans \& Hedrick (2011 [\href={http://dx.doi.org/10.1111/j.1755-0998.2010.02927.x}{doi}]), 
which includes a discussion on this topic. As you'll see in the demonstration
below, the corrected statistics often all tell a similar story. Interestingly, 
...
\section{What statistics can't \textsc{mmod} calculate}
...
rate) into account. It is not clear how the maximum potential value of 
$R_\text{ST}$ for a given dataset can be calculated, so it is not possible to
correct this statistic in a way similar to $G''_\text{ST}$ and 
$\varphi'_\text{ST}$. 
...
(\href{http://www.bentleydrummer.nl/software/software/}{http://www.bentleydrummer.nl/software/software/})
...
package \texttt{pegas}. The corrected statical is simply $\frac{\theta}{\theta_\text{max}}$
...
in a dataset to between-population and within-population components (it is 
...
You probably want to have some idea of how robust this result is. \textsc{mmod} has a
