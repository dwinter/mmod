...
calculate it has its own function:
 
 
<<diffstat>>=
(nc.phi <- Phi_st_Meirmans(nancycats[1:40,]))
@

You might want to see how these different measures compare to each other
across the loci we've looked at. You can see the corrected measures (all those)
other than $G_\text{ST}$ show a similar pattern (Figure 1):
