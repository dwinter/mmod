...
$\phi'\text{ST}$. This statistic is based on the Analysis of Molecular
...
possible to use this frame work for more than two levels of population 
structure, but that has not been implimented in R). Because
 $\phi'\text{ST}$ can take some time to calculate it has its own function:
 
 
<<diffstat>>=
Phi_st_Meirmans(nancycats[1:40,])
@

You might also want to see how Nei's $G_\text{ST}$ compares with
