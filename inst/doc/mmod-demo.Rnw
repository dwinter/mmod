...
for each locus in the dataset. \texttt{H$_S$} and \texttt{H$_T$} are estimates of the 
...
them at the same time. \texttt{Gst} is the standard (Nei) G$_{ST}$, 
\verb+Gprime_st+ is Hedrick's G''$_{ST}$ and \texttt{D} is Jost's d$_{EST}$.
Because all of these statistics are estimated from estimators of \texttt{H$_S$} 
and \texttt{H$_T$}, it's possible to get negative values for each of these 
...
\texttt{D$_{EST}$} is the easiest statistic to interpret, as you expect to 
...
different estimates of divergence but they range from ~0.1--0.4.
\newpage
...
estimates of each of these statistics. For G$_{ST}$ and G''$_{ST}$ these are 
based on the average of \texttt{H$_S$} and \texttt{H$_T$} across loci. For 
D$_{EST}$ you get two, the harmonic mean of the D$_{EST}$ for each locus and, 
because that method won't work if you end up with negative estimates of D$_{EST}$, 
one calculated as per G$_{ST}$ and G''$_{ST}$.
\newpage
...
sample of the \texttt{nancycats} data, then calculate d$_{EST}$ from that sample:
<<bs>>=
...
\texttt{str(bs.D))} or the classic bs.D\$ [tab tab] to check it out. I don't 
